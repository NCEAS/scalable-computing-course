...
5. Add the precipitation Image to the basemap
...
-  Next, set a color palette to use when plotting the data layer. The following is a palette specified for precipitation in the GEE Description tab for ERA5. Here, we adjusted the max value to change the range of pixel values to which the palette should be applied -- this will make our colors stand out a bit more when we layer our precipitation data on our base map, below.
...
Using the following code, we'll learn how to:  

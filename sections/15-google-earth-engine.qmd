...
- get color palette code working and print final map
- incporporate Ingmar's stuff (or keep it separate in scalable-computing-examples)?
- way to print different versions of the map (rather than update all each time a new layer is added)?
:::
...
```{python}
#| eval: false
myMap.setCenter(lat = 60.335998656, lon = -151.87166318, zoom = 4)
myMap
```

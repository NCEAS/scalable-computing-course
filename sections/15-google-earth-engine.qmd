...
::: {.callout-note}
## GEE is just one platform of many.  
Microsoft Planetary Computer, Pangeo, Amazon Web Services (Amazon Enviromental SOmething)
:::
...
::: {.callout-note}
## GEE is just one of a number of cloud platform solutions developed for climate science
:::
...
-  To plot a map over our Google Maps basemap, we need an "Image" rather than an "ImageCollection." ERA5 contains many different climate variables -- explore which variables the dataset contains under the [Bands](https://developers.google.com/earth-engine/datasets/catalog/ECMWF_ERA5_DAILY#bands) tab. We'll use the `.select` method to choose the parameter(s) (in this case, `total_precipitation`) we're interested in from our `weatherData` object.

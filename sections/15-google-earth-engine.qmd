...
-  We can use the `setCenter` method again to center our map, `myMap` over an area of interest -- here, we've chosen coordinates for Cook Inlet, Alaska. `setCenter` takes the longitude and latitude as the first two arguments, followed by the zoom level.

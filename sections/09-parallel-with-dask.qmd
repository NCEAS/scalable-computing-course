...
Both tif files are 76 MB, so let us have chunks of 12 MB to have roughly 6 chunks.
...
red = da.from_array(fp_red, chunks = 1000)
red
```

In the information above we can see how many chunks were created and some basic information about the array’s dimensions. 

```{python}
fp_nir = os.path.join(os.getcwd(),"RU_ANS_TR2_FL005M_nir.tif")
nir = da.from_array(fp_nir)
```

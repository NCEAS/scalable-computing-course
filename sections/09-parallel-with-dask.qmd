...
Dask automatically chunks up the data to create the `dask.array`, but we can manually specify either a shape or the size in bytes for each chunk. 
Both tif files are 76 MB, so let us have chunks of 15 MB to have roughly 6 chunks.
...
```{python}
url = "https://arcticdata.io/metacat/d1/mn/v2/object/urn%3Auuid%3A9a078f8d-32a7-4335-98c1-a31d684c4d7a"
response = requests.get(url)
open("RU_ANS_TR2_FL005M_ndvi.tif", "wb").write(response.content)
```

```{python}
ndvi_x =  xr.open_rasterio('RU_ANS_TR2_FL005M_ndvi.tif')
ndvi_x = ndvi.squeeze()
ndvi_x.plot(robust=True)
```

```{python}
# red_x =  xr.open_rasterio(fp_red)
# nir_x = xr.open_rasterio(fp_nir)
# red_x
ndvi_x.plot()
```

```{python}
red_x.plot(robust=True)
```

```{python}
nir_x.plot(robust=True)
```

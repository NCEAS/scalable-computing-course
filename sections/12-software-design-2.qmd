...
To get started, we're using `virtualenvwrapper` to manage virtual environments in the course, so let's be sure it is installed in your environment. We're starting from the `scalable-computing-examples` workspace, so you should be in the `scomp` virtual environment in your terminal. So, make sure virtualenvwrapper is installed, and install it if not in `scomp`, using:
...
- Open a new VS Code window by connecting to included-crab
- Command-Shift-P "Connect to Host..."
  - select included-crab
- Open a new Terminal if one doesn't open
- Switch to our new venv
...
```
- Close the window, and open "New Window" under VS Code, then using `Ctrl-R` shortcut, open the recent admodel workspace on included crab
  - if this all goes well, your session should be set to open up this `adcmodel` each time that you open the workspace

...
def run_tasks_s(task_list):
    result = [task(x) for x in task_list]
    return result

results = run_tasks_s([1.0, 1.1, 1.2, 1.3])
[x for x in results]
```
...
def run_tasks_p(task_list):
    with ThreadPoolExecutor(max_workers=5) as cf_executor:
        return cf_executor.map(task, task_list)

results = run_tasks_p([1.0, 1.1, 1.2, 1.3])
[x for x in results]
```
...
def download_list(dl_list):
    with ThreadPoolExecutor(max_workers=15) as cf_executor:
        return cf_executor.map(download_file, dl_list.itertuples(), timeout=60)
...
def download_list(dl_list):
    """
    Download data files using parsl
    """
    results = []
    for row in dl_list.itertuples():

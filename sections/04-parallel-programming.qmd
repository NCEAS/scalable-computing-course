...
Python also provides a number of easy to use packages for concurrent processing. We will review two of these, `concurrent.futures` and `parsl`, to show just how easy it can be to parallelize your programs. 
...
results = download_serial(file_table[0:1])
print(results)
```

In this code, we have one function (`download_file`) that downloads a single data file and saves it to disk. It is called iteratively from the function `download_list`. The serial execution takes about 15.2 seconds, but can vary by a few seconds.
...
results = download_threaded(file_table[0:1])
...
results = download_process(file_table[0:1])
for result in results:
	print(result)
```

### Multi-process with `parsl`

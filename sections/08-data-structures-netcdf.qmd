...
# time coordinates are straight-forward:
time_coords = pd.date_range("2022-09-01", "2022-09-03")

# for the lat and lon coordinates we need to remember the following:
# we are assigning values to the indices of temp_data, which is a numpy.array 
# this means that the indexing in every 2-dimensional array:
#       - starts at the top-left corner, and
#       - is ordered by rows first, columns second (like matrices)
# then: 

# lat (y-axis) values start at the top of the array and go down
# so we arange the lat values in decreasing order: 70, 60, 50, 40, 30:
lat_coords = np.arange(70, 20, -10)

# lon (x-axis) values start at the left of the array and go right
# so we arange the lon values in increasing order: 60, 70, 80, 90, 100:
lon_coords = np.arange(60, 110, 10)

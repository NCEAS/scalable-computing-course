...
You can find all of the CF convention's standard names [in this table](http://cfconventions.org/Data/cf-standard-names/current/build/cf-standard-name-table.html).
Variables should also include a `units` attribute. This should be a string that can be recognized by UNIDATA’s Udunits package [UDUNITS](https://www.unidata.ucar.edu/software/udunits/).
The North Carolina Institute for Climate Studies holds a [list of the units found in the UDUNITS database](https://ncics.org/portfolio/other-resources/udunits2/#degree_N).
...
Now imagine we calculate the average temperature over time at each weather station and we wish to incorporate ths data into the same dataset.
How will this change the variables, attributes and dimensions of the dataset?
...
**Attributes**:
We need to keep all the previous attributes for the variables, dimension and dataset. 
...
In this last section, we will go over the relation between tabular data and the NetCDF data format and how to transform a `pandas.DataFrame` into an `xarray.DataSet`.

### Tabular to NetCDF

We assume our starting point is tabular data which meets the criteria for **tidy data**, which means:
...
The diagram below shows how these columns would transform into variables, dimensions and coordinates. 

![](../images/netcdf_pandas_to_xarray.png)

Tabular formats like csv do not offer a standard way to encode attributes for the dimensions or variables, this is why we don't see any attributes in the resulting NetCDF data. One of the biggest advantages of NetCDF is its self-describing properties.

### `pandas` to `xarray`

How does the previous example look like when we are working with `pandas` and `xarray`?
...
For further reading and examples about switching between `pandas` and `xarray` be sure to visit the following:

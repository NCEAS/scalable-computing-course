...
NetCDF files are designed to be [[1]](https://docs.unidata.ucar.edu/netcdf-c/current/faq.html#ncFAQGeneral):
...
On the first day, all stations record a temperature of 0°C. On the second day, all temperatures are 1°C, and on the third day, all temperatures are 3°C. 
...
In Python, the [`NumPy`](https://numpy.org) package provides the fundamental data structure and API for working with raw ND arrays. 
...
Additionally, each dimension has at least one set of *coordinates*, indicating the dimension's values. 
...
An `xarray.DataSet` resembles an in-memory representation of a NetCDF file and consists of *multiple* variables, with coordinates and attributes, forming a self-describing dataset.
We can create an `xarray.DataSet` by combining the temperature data with the average temperature data. 

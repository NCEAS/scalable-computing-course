...
You may have noticed that a netCDF dimension only specifies the _name_ and _length_ of a given variable axis. How do we know the _values_ of the individual coordinates along each dimension? The solution is to create an additional one-dimensional variable for each dimension, containing the values corresponding to positional along the dimension. This variable should have the same name as the dimension itself, contain values in increasing or decreasing order, and contain no missing values. In addition, standard practice is to minimally provide `unit` and `long_name` attributes for coordinate variables to make them interpretable. Although not strictly part of the netCDF specification, the creation of coordinate variables is considered a [best practice](https://docs.unidata.ucar.edu/nug/current/best_practices.html#bp_Coordinate-Systems) is an important convention in the netCDF community, and netCDF software libraries are built to detect and leverage this information.
...
What are the *variables*, *dimensions* and *attributes* for this data, including the coordinate variables for each dimension?
